...
//       setReturnedImages([]); // Reset returned images when a new image is uploaded
//     };
...
//       setReturnedImages([url]); // This will be the original image
...
//     if (imageURL) {
//       runBodysegment();
//       setReturnedImages([]); // Clear returned images on reset
//     }
//   };
...
//       return "Original";
//     } else {
//       const dataIndex = index - 1; // Adjust index since first image is original
//       const monthLabel = weightLossData[dataIndex]?.month;
//       return monthLabel || getOrdinal(index);
//     }
//   };
...
//                       <td>{row.bmi}</td>
//                       <td><button onClick={() => handleShowClick(row.monthNumber)}>Show</button></td>
//                     </tr>
//                   ))}
//                 </tbody>
//               </table>
...
function App() {
  ...
  // Function to notify the server when the user accesses the page
  const notifyAccess = useCallback(async () => {
    try {
      await fetch('https://20.50.198.244:8080/user_access', {
        method: 'POST',
  ...
  // Function to notify the server when the user leaves the page
  const notifyExit = useCallback(() => {
    // Use navigator.sendBeacon for reliable sending during unload
    const url = 'https://20.50.198.244:8080/user_exit';
    const data = new Blob([], { type: 'application/json' });
    navigator.sendBeacon(url, data);
  }, []);

  useEffect(() => {
    // Notify the server when the page loads
    notifyAccess();
    ...
    return () => {
      ...
      // Optionally, notify the server here as well
    };
  }, [notifyAccess, notifyExit]);
              ...
              <div className="bmi-section">
                <div className="bmi-form">
                  ...
                  <input
                    ...
                    onChange={handleDesiredBmiChange}
                  />
                  <button onClick={handleCalculateBmi} className="calculate-button">
                    Calculate
                  </button>
                  {bmi && (
                    <div className="bmi-result">
                ...
                <tbody>
                  {weightLossData.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td>{row.bmi}</td>
                      <td>
                        <button onClick={() => handleShowClick(row.monthNumber)}>Show</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
